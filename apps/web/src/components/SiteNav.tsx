...
interface SiteNavProps {
    /** Show the nav links (default true). Set to false for focused flows like /prove. */
    showLinks?: boolean;
    wide?:      boolean;
    badge?:     string;
}

export function SiteNav({ showLinks = true, wide = false, badge = "beta" }: SiteNavProps) {
        ...
        <nav className="sticky top-0 z-50 border-b border-border bg-bg/90 backdrop-blur-sm">
            <div className={`${maxW} mx-auto px-6 h-14 flex items-center justify-between`}>
                ...
                <div className="flex items-center gap-0.5">
                    {showLinks && NAV_LINKS.map(({ href, label }) => {
                    ...
                    <div className={`${showLinks ? "ml-3 pl-3 border-l border-border" : ""}`}>
                        <ThemeToggle />
                    </div>
                </div>
